...
OpenVINOExecutionProvider::OpenVINOExecutionProvider(const OpenVINOExecutionProviderInfo& info)
  ...
  InitProviderOrtApi();
  ////
  get_mem_custom("OpenVINOExecutionProvider(const OpenVINOExecutionProviderInfo& info)");
  ///
  ...
  if (info.use_compiled_network_ == false) {
    ...
    ////
    get_mem_custom("ov_interface.cc --> L41 After openvino_ep::BackendManager::GetGlobalContext().ie_core.GetAvailableDevices();");
    ////
...
common::Status OpenVINOExecutionProvider::Compile(
  ...
  for (const auto& fused_node_graph : fused_nodes) {
    ...
    ////
    get_mem_custom("openvino_execution_provider.cc --> L153 Inside common::Status OpenVINOExecutionProvider::Compile;");
    ////
