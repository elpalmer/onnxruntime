...
BasicBackend::BasicBackend(const ONNX_NAMESPACE::ModelProto& model_proto,
  ...
  ////
  get_mem_custom("basic_backend.cc --> L40 BEFORE CreateOVModel");
  ////
  if (hw_target == "MYRIAD")
    vpu_status = true;
  if (!ImportBlob(hw_target, vpu_status)) {
    ...
    ////
    get_mem_custom("basic_backend.cc --> L40 AFTER CreateOVModel");
    ////
...
// Setting OpenCL queue throttling for GPU
  ...
  ////
  get_mem_custom("basic_backend.cc --> L77 After ie_core.LoadNetwork(...)");
  ////
...
void BasicBackend::CompleteAsyncInference(Ort::KernelContext& context, OVInferRequestPtr infer_request) {
  ...
  ////
  get_mem_custom("basic_backend.cc --> L388 Begin of CompleteAsyncInference(Ort::KernelContext& context, OVInferRequestPtr infer_request)");
  ////
  infer_request->WaitRequest();
#if defined(OV_API_20)
  auto graph_output_info = exe_network_.Get().outputs();
  ////
  get_mem_custom("basic_backend.cc --> L394 auto graph_output_info = exe_network_.Get().outputs();");
  ////
  for (auto output_info_iter = graph_output_info.begin();
       output_info_iter != graph_output_info.end(); ++output_info_iter) {
    ////
    get_mem_custom("basic_backend.cc --> L396 BEFORE     OVTensorPtr graph_output_blob;");
    ////
    OVTensorPtr graph_output_blob;
    ////
    get_mem_custom("basic_backend.cc --> L396 AFTER     OVTensorPtr graph_output_blob;");
    ////
    auto output_names = output_info_iter->get_names();
    std::string onnx_output_name;
    std::string output_name;
    ...
    if (mem_info.GetAllocatorName() == OpenVINO_GPU) {
      ...
      size_t batch_slice = 0;
      ////
      get_mem_custom("basic_backend.cc --> L428 BEFORE FillOutputBlob(graph_output_blob, output_tensor, batch_slice);");
      ////
      FillOutputBlob(graph_output_blob, output_tensor, batch_slice);
      ////
      get_mem_custom("basic_backend.cc --> L428 AFTER FillOutputBlob(graph_output_blob, output_tensor, batch_slice);");
      ////
    }
  }
#else
...
#endif
    ////
    get_mem_custom("basic_backend.cc --> L506 BEFORE CompleteAsyncInference(context, infer_request);");
    ////
    CompleteAsyncInference(context, infer_request);
    ////
    get_mem_custom("basic_backend.cc --> L506 AFTER CompleteAsyncInference(context, infer_request);");
    ////
