...
BasicBackend::BasicBackend(const ONNX_NAMESPACE::ModelProto& model_proto,
  ...
  if (hw_target == "MYRIAD")
    vpu_status=true;
  try{
    import_blob = ImportBlob(hw_target, vpu_status);
  ...
  if (!import_blob) {
    ...
    try{
      ie_cnn_network_ = CreateOVModel(model_proto, global_context_, subgraph_context_, const_outputs_map_);
        ...
        // std::cout << " propogated from backend_utils createovmodel() " << std::endl;
        throw msg;
    }
  #endif
  ...
  try{
    ...
    if (!const_outputs_map_.empty()) {
      for (auto item : const_outputs_map_) {
        ...
        auto mem_info = ort.GetTensorMemoryInfo(output_tensor);
        if (strcmp(mem_info->name, OpenVINO_GPU) == 0) {
          throw std::string(log_tag + "IO Buffering is not supported for constant subgraphs");
        } else {
